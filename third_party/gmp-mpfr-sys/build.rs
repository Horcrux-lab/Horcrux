...
fn get_actual_cross_target(cross_target: &str) -> &str {
    if cross_target == "aarch64-apple-ios-sim" { return "aarch64-apple-darwin"; }
    match cross_target {
...
fn build_gmp(env: &Environment, lib: &Path, header: &Path) {
    ...
    let is_ios = env.cross_target.as_ref().map_or(false, |t| t.contains("apple-ios"));
    // On iOS we must disable both fat (CPU dispatch) and assembly, because
    // GMP's aarch64 asm uses non-GOT-relative ADRP relocations that iOS
    // clang's integrated assembler rejects. Fall back to portable C.
    let mut conf = if is_ios {
        String::from("../gmp-src/configure --disable-shared --with-pic --disable-assembly")
    } else {
        String::from("../gmp-src/configure --enable-fat --disable-shared --with-pic")
    };
