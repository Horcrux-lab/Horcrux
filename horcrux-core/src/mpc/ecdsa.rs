...
//!
//! ## Presigning (offline/online split)
//!
//! For low-latency signing, the protocol supports a presign + sign split:
//!
//! 1. **Presign (offline, MPC 3 rounds)**: `EcdsaPresignSession` runs
//!    `generate_presignature_sync` producing a `Presignature` per party.
//! 2. **Sign (online, 1 broadcast round)**: `EcdsaPresignedSigningSession`
//!    each party calls `issue_partial_signature` locally, broadcasts partial
//!    signatures, then any party calls `PartialSignature::combine`.
//!
//! ⚠️ **Never reuse presignatures!** Reuse leaks the private key.
...
/// Create a presignature generation state machine driver.
fn make_presign_driver(
    our_signing_index: u16,
    parties_at_keygen: &[u16],
    key_share: &cggmp21::KeyShare<Secp256k1, SecurityLevel128>,
) -> Result<Box<dyn AnyDriver>, MpcError> {
    let eid_bytes: &'static [u8] = Box::leak(
        format!("horcrux-presign-{}", uuid_v4())
            .into_bytes()
            .into_boxed_slice(),
    );
    let eid = ExecutionId::new(eid_bytes);
    ...
    let sm = cggmp21::signing(eid, our_signing_index, parties, ks)
        .generate_presignature_sync(rng);
...
// =============================================================================
// ECDSA Presign Session (offline phase)
// =============================================================================

/// Presign session: runs the MPC presignature generation protocol (3 rounds).
///
/// Produces a `Presignature` per party that can be cached and later used
/// for 1-round online signing via `EcdsaPresignedSigningSession`.
pub struct EcdsaPresignSession {
    config: HorcruxConfig,
    session_id: String,
    driver: Box<dyn AnyDriver>,
    group_public_key: NonZero<Point<Secp256k1>>,
    complete: bool,
    result: Option<Vec<u8>>, // serialized Presignature
}

impl std::fmt::Debug for EcdsaPresignSession {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EcdsaPresignSession")
            .field("session_id", &self.session_id)
...
impl EcdsaPresignSession {
    pub fn new(
        ...
        let driver = make_presign_driver(our_signing_index, &parties_at_keygen, &key_share)?;
    ...
    fn drain_outbox(&mut self) -> Result<Vec<MpcMessage>, MpcError> {
        ...
        loop {
            match self.driver.drive()? {
                ...
                DriverAction::NeedMessage => break,
                DriverAction::Complete(output_bytes) => {
                    self.result = Some(output_bytes);
                    self.complete = true;
                    tracing::info!(
                        party = self.config.party_index,
                        "ECDSA presignature generated — ready for 1-round signing"
                    );
                    break;
                }
            }
    ...
    /// Get the serialized presignature if complete.
    pub fn presignature_bytes(&self) -> Option<&[u8]> {
        self.result.as_deref()
    }

    /// Get the group public key (for later recovery_id computation).
    pub fn group_public_key(&self) -> &NonZero<Point<Secp256k1>> {
        &self.group_public_key
    }
...
// =============================================================================
// ECDSA Presigned Signing Session (online phase — 1 broadcast round)
// =============================================================================

/// Wire message for the presigned signing phase (partial signature exchange).
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct PartialSigWireMsg {
    pub from: u16,
    pub r: Vec<u8>,
    pub sigma: Vec<u8>,
}

/// Online signing using a cached presignature.
///
/// Flow:
/// 1. Each party calls `start()` which locally computes `issue_partial_signature`
///    and broadcasts the partial signature to all other parties.
/// 2. Each party calls `process_message()` for each received partial signature.
/// 3. When threshold partial signatures are collected, `combine()` produces
///    the final signature.
///
/// Total: **1 broadcast round** (vs 4 rounds for full signing).
pub struct EcdsaPresignedSigningSession {
    config: HorcruxConfig,
    session_id: String,
    group_public_key: NonZero<Point<Secp256k1>>,
    message_hash: Vec<u8>,
    /// Our own partial signature (computed locally on start)
    our_partial: Option<cggmp21::PartialSignature<Secp256k1>>,
    /// Collected partial signatures (including ours)
    partials: Vec<cggmp21::PartialSignature<Secp256k1>>,
    result: Option<SigningResult>,
}

impl std::fmt::Debug for EcdsaPresignedSigningSession {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EcdsaPresignedSigningSession")
            .field("session_id", &self.session_id)
...
impl EcdsaPresignedSigningSession {
    /// Create a presigned signing session.
    ///
    /// `presignature_bytes`: serialized `Presignature<Secp256k1>` from presign phase
    /// `group_public_key`: the group's public key (from DKG/presign)
    pub fn new(
        config: HorcruxConfig,
        message_hash: Vec<u8>,
        ...
        // Deserialize and immediately consume the presignature
        let presignature: cggmp21::Presignature<Secp256k1> =
            serde_json::from_slice(presignature_bytes)
                .map_err(|e| MpcError::SigningFailed(format!("deserialize presignature: {e}")))?;

        let data_to_sign = cggmp21::DataToSign::from_scalar(
            Scalar::<Secp256k1>::from_be_bytes_mod_order(&message_hash),
        );

        // Issue partial signature locally — no MPC needed!
        ...
        Ok(Self {
            ...
            group_public_key,
            message_hash,
            our_partial: Some(partial),
            partials: Vec::new(),
            result: None,
        })
    }

    /// Start the online signing: broadcast our partial signature.
    pub fn start(&mut self, session_id: &str) -> Result<Vec<MpcMessage>, MpcError> {
        self.session_id = session_id.to_string();

        let partial = self.our_partial.take()
            .ok_or_else(|| MpcError::SigningFailed("already started".into()))?;

        // Add our own partial to the collection
        self.partials.push(partial.clone());

        // Serialize and broadcast to all other parties
        let wire = PartialSigWireMsg {
            from: self.config.party_index,
            r: serde_json::to_vec(&partial.r)
                .map_err(|e| MpcError::ProtocolError(format!("serialize r: {e}")))?,
            sigma: serde_json::to_vec(&partial.sigma)
                .map_err(|e| MpcError::ProtocolError(format!("serialize sigma: {e}")))?,
        };

        let outer = EcdsaWireMsg {
            from: self.config.party_index,
            phase: EcdsaPhase::PresignedSign,
            is_broadcast: true,
            payload: serde_json::to_vec(&wire)
                .map_err(|e| MpcError::ProtocolError(format!("serialize wire: {e}")))?,
        };
        let payload = serde_json::to_vec(&outer)
            .map_err(|e| MpcError::ProtocolError(format!("serialize outer: {e}")))?;
    ...
    /// Process an incoming partial signature from another party.
    pub fn process_message(&mut self, msg: MpcMessage) -> Result<Vec<MpcMessage>, MpcError> {
        let outer: EcdsaWireMsg = serde_json::from_slice(&msg.payload)
            .map_err(|e| MpcError::ProtocolError(format!("deserialize outer: {e}")))?;
        let wire: PartialSigWireMsg = serde_json::from_slice(&outer.payload)
            .map_err(|e| MpcError::ProtocolError(format!("deserialize partial: {e}")))?;

        let r: Scalar<Secp256k1> = serde_json::from_slice(&wire.r)
            .map_err(|e| MpcError::ProtocolError(format!("deserialize r: {e}")))?;
        let sigma: Scalar<Secp256k1> = serde_json::from_slice(&wire.sigma)
            .map_err(|e| MpcError::ProtocolError(format!("deserialize sigma: {e}")))?;

        self.partials.push(cggmp21::PartialSignature { r, sigma });

        // Try to combine once we have threshold partial signatures
        if self.partials.len() >= self.config.threshold as usize && self.result.is_none() {
            self.try_combine()?;
        }

        Ok(Vec::new()) // no further messages needed
    }

    fn try_combine(&mut self) -> Result<(), MpcError> {
        let signature = cggmp21::PartialSignature::combine(&self.partials)
            .ok_or_else(|| MpcError::SigningFailed("failed to combine partial signatures".into()))?;
        ...
        tracing::info!(
            party = self.config.party_index,
            recovery_id,
            "ECDSA presigned signing complete (1-round)"
        );
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn test_presign_session_creation() {
        // Can't create without valid shard data, but test the error path
        let config = HorcruxConfig::new(2, 3, 1, CurveType::Secp256k1).unwrap();
        let result = EcdsaPresignSession::new(config, vec![1, 2, 3], vec![1, 2, 3]);
        assert!(result.is_err()); // invalid shard data
    }

    #[test]
    fn test_presign_session_insufficient_parties() {
        let config = HorcruxConfig::new(2, 3, 1, CurveType::Secp256k1).unwrap();
        let result = EcdsaPresignSession::new(config, vec![], vec![1]); // threshold=2, got 1
        assert!(result.is_err());
    }

    #[test]
    fn test_presigned_signing_invalid_presignature() {
        let config = HorcruxConfig::new(2, 3, 1, CurveType::Secp256k1).unwrap();
        let gen_point: Point<Secp256k1> = Point::generator().to_point();
        let pk = NonZero::from_point(gen_point).unwrap();
        let result = EcdsaPresignedSigningSession::new(
            config,
            vec![0u8; 32],
            &[1, 2, 3], // invalid presignature bytes
            pk,
        );
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_partial_sig_wire_msg_serialization() {
        let wire = PartialSigWireMsg {
            from: 1,
            r: vec![1, 2, 3],
            sigma: vec![4, 5, 6],
        ...
        let bytes = serde_json::to_vec(&wire).unwrap();
        let decoded: PartialSigWireMsg = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(decoded.from, 1);
        assert_eq!(decoded.r, vec![1, 2, 3]);
        assert_eq!(decoded.sigma, vec![4, 5, 6]);
    }
}
