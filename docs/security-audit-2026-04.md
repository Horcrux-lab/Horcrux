...
- **iOS call-sites (round 14)**:
  ...
  - **Refresh** (`RefreshShardCoordinator`): explicit peer-registry (round 16). Each wallet persists the `[peer.id: party_index]` map that was assigned deterministically at DKG time (`Wallet.peerRegistry`, also mirrored into `AccountBackup` v4+). Refresh loads this registry up-front and enforces it in strict mode — an inbound claim from a peer.id outside the DKG roster is rejected, and a claim that disagrees with the pinned index is rejected as impersonation. Legacy wallets created before round 16 (`peerRegistry == nil`) fall back to TOFU-per-session and, on the first successful refresh, have the observed map persisted via `WalletStore.setPeerRegistryIfAbsent` so every subsequent ceremony runs in strict mode without TOFU.
  ...
  - **Cold signing v2** (`ColdSigningCoordinatorV2`): t-of-n star-topology — enforces `msg.fromParty != myIndex` (self-impersonation), then binds `authenticatedFrom = msg.fromParty`. The QR-scan visual verification remains the channel authentication. **Round 15 follow-up (closed)**: also enforces (a) `msg.sessionId` matches the coordinator's active ceremony, (b) per-party sessionId stickiness (TOFU — first-seen sessionId for each `fromParty` is frozen, later divergence = ceremony splice, rejected), and (c) writes a per-party scan-session fingerprint `SHA256(sessionId ‖ fromParty_be ‖ first_payload)` to SecureLog for audit-export forensic traceability.
