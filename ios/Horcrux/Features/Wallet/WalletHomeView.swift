...
struct WalletHomeView: View {
    ...
    /// Aggregate security health used by the home-screen `SecurityHealthCard`.
    /// Fold multiple signals into a single three-state traffic light so users
    /// can see "am I safe?" at a glance, without having to dig through
    /// Settings / Shards / Rotate subscreens.
    ///
    /// Signals (today):
    /// - **Rotation**: attention when any CGGMP21 account is past the
    ///   recommended interval; risk when it's been ≥ 180d since the last
    ///   rotation (or "never" combined with a wallet >90d old).
    /// - **Local biometric backup (SE-sealed SWK)**: attention when biometric
    ///   is available on the device but the user never sealed an SWK copy,
    ///   because losing the PIN then means permanent device data loss.
    ///
    /// Overall level is the max of the individual signals.
    private enum HealthLevel: Int, Comparable {
    ...
    private struct SecurityHealth {
        let overall: HealthLevel
        let rotation: HealthLevel
        let rotationDaysText: String?  // "32 天前" / "从未轮换" / nil if irrelevant
        let hasSealedBackup: Bool
        let backupAttention: Bool      // true when .attention due to missing SE seal
        let staleWalletForCTA: Wallet? // existing staleWallet, surfaced for Rotate CTA
    }

    private func computeSecurityHealth() -> SecurityHealth {
        let stale = staleWallet
        ...
        var rotationLvl: HealthLevel = .safe
        ...
        // Only flag missing backup when the device can actually support
        // biometric — on sim / older devices there's nothing to seal against.
    ...
    private var securityHealthCard: some View {
        // Hide the card entirely when nothing is actionable AND the user has
        // snoozed any prior attention prompt in the last 24h — same snooze
        // budget the old rotation nudge honoured, now shared across all
        // security signals so we don't pop back in as soon as another one
        // goes yellow.
        let snoozed = rotationNudgeDismissedAt.map { Date().timeIntervalSince($0) < 24 * 3600 } ?? false
        let health = computeSecurityHealth()

        if !walletStore.wallets.filter({ !$0.hidden }).isEmpty,
           health.overall != .safe || !snoozed {
            VStack(alignment: .leading, spacing: 12) {
                // Header: traffic-light + state label
                HStack(spacing: 10) {
                ...
                // Row 2 — local biometric backup (SE-sealed SWK)
                securityRow(
            ...
            .background(
                RoundedRectangle(cornerRadius: 14)
                    .fill(health.overall.tint.opacity(0.10))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 14)
                    .stroke(health.overall.tint.opacity(0.35), lineWidth: 1)
            )
        }
    }
